...
keep_degs <- read.csv("all_DEGs_cardio_rep.csv")
rownames(keep_degs) <- keep_degs[,1]

data_counts <- read.csv("Final_cardio.csv")
...
annotation <- read.csv("enst_to_symbol.csv",sep = ",", header = TRUE)
...
write.csv(edges, file="Cardio_edges.csv", quote = FALSE,row.names = F)
write.csv(nodes, file="Cardio_nodes.csv", quote = FALSE,row.names = F)