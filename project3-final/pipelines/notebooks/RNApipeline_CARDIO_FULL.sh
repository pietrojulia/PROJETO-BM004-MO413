...
# run alignment
hisat2 -p 4 -q -x HISAT2/Homo_sapiens/genome --dta -1 Trimmed_Data/"${file_name}_forward_paired.fastq" -2 Trimmed_Data/"${file_name}_reverse_paired.fastq" | samtools sort -o HISAT2/Cardiomiocitos/"${file_name}.bam"
...
stringtie -p 4 HISAT2/Cardiomiocitos/"${file_name}.bam" -G Gene_anotation/Homo_sapiens.GRCh38.115.gtf -o counts/Cardiomiocitos/"${file_name}.gtf"

done

# STEP 4: Merge

ls counts/Cardiomiocitos/*.gtf > counts/Cardiomiocitos/gtfs.txt

stringtie --merge -G Gene_anotation/Homo_sapiens.GRCh38.115.gtf -o counts/Cardiomiocitos/cardio_merged.gtf $(cat counts/Cardiomiocitos/gtfs.txt)
